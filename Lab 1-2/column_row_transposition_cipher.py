...
class ColumnRowTranspositionCipher:
    ...
    def decrypt(self,
        ...
        row_key_indices_inverse = [row_key_indices.index(i) for i in range(len(row_key_indices))]

        # Rearrange the columns based on the sorted column key
        column_key_indices = [column_key.index(letter) for letter in sorted(column_key)]
        column_key_indices_inverse = [column_key_indices.index(i) for i in range(len(column_key_indices))]
        ...
        decrypted_message = "".join(matrix[row_key_indices_inverse[row]][column_key_indices_inverse[column]]
                                    for row in range(num_rows) for column in range(num_columns))
...
if __name__ == "__main__":
    ...
    column_row_transposition_cipher = ColumnRowTranspositionCipher(column_key=_column_key,
                                                                   row_key=_row_key)
